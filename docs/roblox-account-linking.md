...
**Consent prompt is required for this request** occurs when requesting account selection alone; **Account selection prompt is required for this request** occurs when requesting consent alone. The backend requests both with a single `prompt=consent select_account` parameter, using the [space-delimited OpenID Connect format](https://openid.net/specs/openid-connect-core-1_0.html#AuthRequest). Deploy this code to the host's `main` branch and restart to pull/rebuild, then generate a new link; an already-open authorization URL still contains the old prompt. These errors are separate from the private-app user limit.
